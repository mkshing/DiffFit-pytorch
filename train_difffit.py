...
---
    ...
    model_card = f"""
# DiffFit - {repo_id}
These are DiffFit weights for {base_model}. The weights were trained on {prompt}.
"""
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--train_text_encoder",
        action="store_true",
        help="Whether to adapt DiffFit to the text encoder. If set, the text encoder should be float32 precision.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="loss = loss + lambda * loss_vlb"
    )
    parser.add_argument(
        "--bitfit",
        action="store_true",
        default=False,
        help=(
            "Use BitFit setting instead of DiffFit"
        ),
    )
...
def get_lvlb_weights(noise_scheduler):
    ...
    if noise_scheduler.config.prediction_type == "epsilon":
        lvlb_weights = noise_scheduler.betas ** 2 / (2 * posterior_variance * to_torch(alphas) * (1 - alphas_cumprod))
    elif noise_scheduler.config.prediction_type == "v_prediction":
        lvlb_weights = torch.ones_like(betas ** 2 / (2 * posterior_variance * to_torch(alphas) * (1 - alphas_cumprod)))
    else:
...
def main(args):
    ...
    if args.add_vlb_loss:
        lvlb_weights = get_lvlb_weights(noise_scheduler).to(accelerator.device)
    text_encoder = load_text_encoder_for_difffit(args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision, is_bitfit=args.bitfit)
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision)
    unet = load_unet_for_difffit(args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, is_bitfit=args.bitfit)

    vae.requires_grad_(False)
    if not args.train_text_encoder:
        text_encoder.requires_grad_(False)
    else:
        text_encoder = mark_only_biases_as_trainable(text_encoder, is_bitfit=args.bitfit)
    unet = mark_only_biases_as_trainable(unet, is_bitfit=args.bitfit)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("difffit-pytorch", config=vars(args))
            ...
            with accelerator.accumulate(unet):
                ...
                if args.with_prior_preservation:
                    ...
                    if args.add_vlb_loss:
                        ...
                        loss = loss + args.vlb_lambda * loss_vlb
